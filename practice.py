...
def long_words(words):
    ...
    If no words are longer than 4 characters, return an empty list::
        ...
        []
    """
# Including 2 ways to solve this - I like list comprehensions :)
#    long_word_list = []
...
#        numbers_halved.append(float(number) / 2)
...
# This seems like it could be more elegant, but leaving as-is in the interests
# of time, and MVP...!!
    ...
    for word in words:
        if len(words) == 1:
            ...
            if index < (len(words) - 1):
                new_word = new_word + ", " + words[index + 1]
                index += 1
...
def reverse_list_in_place(orig):
...
def duplicates(items):
    ...
    for item in items:
        if items.count(item) > 1 and item not in dup_list:
